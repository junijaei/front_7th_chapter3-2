import { atom } from 'jotai';
import { atomWithStorage } from 'jotai/utils';
import {
  addDiscountToList,
  addProductToList,
  Discount,
  Product,
  ProductValidation,
  removeDiscountFromList,
  removeProductInList,
  updateProductInList,
  updateStockInList,
  validateAddProduct,
  validateRemoveCartItem,
  validateUpdateProduct,
} from '@/features/product';

const initialProducts: Product[] = [
  {
    id: 'p1',
    name: '상품1',
    price: 10000,
    stock: 20,
    discounts: [
      { quantity: 10, rate: 0.1 },
      { quantity: 20, rate: 0.2 },
    ],
    description: '최고급 품질의 프리미엄 상품입니다.',
  },
  {
    id: 'p2',
    name: '상품2',
    price: 20000,
    stock: 20,
    discounts: [{ quantity: 10, rate: 0.15 }],
    description: '다양한 기능을 갖춘 실용적인 상품입니다.',
    isRecommended: true,
  },
  {
    id: 'p3',
    name: '상품3',
    price: 30000,
    stock: 20,
    discounts: [
      { quantity: 10, rate: 0.2 },
      { quantity: 30, rate: 0.25 },
    ],
    description: '대용량과 고성능을 자랑하는 상품입니다.',
  },
];

// Base atom
export const productsAtom = atomWithStorage<Product[]>(
  'products',
  initialProducts
);

// Action atoms
export const addProductAtom = atom(
  null,
  (
    get,
    set,
    {
      newProduct,
      options,
    }: {
      newProduct: Omit<Product, 'id'>;
      options?: {
        onSuccess?: (validation: ProductValidation) => void;
        onError?: (validation: ProductValidation) => void;
      };
    }
  ) => {
    const products = get(productsAtom);
    const result = validateAddProduct(products, newProduct);

    if (!result.valid) {
      options?.onError?.(result);
      return;
    }

    set(productsAtom, (prev) => addProductToList(prev, newProduct));
    options?.onSuccess?.(result);
  }
);

export const updateProductAtom = atom(
  null,
  (
    get,
    set,
    {
      product,
      options,
    }: {
      product: Partial<Product>;
      options?: {
        onSuccess?: (validation: ProductValidation) => void;
        onError?: (validation: ProductValidation) => void;
      };
    }
  ) => {
    if (!product.id) {
      options?.onError?.({
        valid: false,
        error: 'NOT_FOUND',
        message: '상품 ID가 필요합니다.',
      });
      return;
    }

    const products = get(productsAtom);
    const result = validateUpdateProduct(products, product.id, product);

    if (!result.valid) {
      options?.onError?.(result);
      return;
    }

    set(productsAtom, (prev) => updateProductInList(prev, product));
    options?.onSuccess?.(result);
  }
);

export const deleteProductAtom = atom(
  null,
  (
    get,
    set,
    {
      productId,
      options,
    }: {
      productId: string;
      options?: {
        onSuccess?: (validation: ProductValidation) => void;
        onError?: (validation: ProductValidation) => void;
      };
    }
  ) => {
    const products = get(productsAtom);
    const result = validateRemoveCartItem(products, productId);

    if (!result.valid) {
      options?.onError?.(result);
      return;
    }

    set(productsAtom, (prev) => removeProductInList(prev, productId));
    options?.onSuccess?.(result);
  }
);

export const updateProductStockAtom = atom(
  null,
  (_get, set, { productId, stock }: { productId: string; stock: number }) => {
    set(productsAtom, (prev) => updateStockInList(prev, productId, stock));
  }
);

export const addProductDiscountAtom = atom(
  null,
  (
    _get,
    set,
    { productId, discount }: { productId: string; discount: Discount }
  ) => {
    set(productsAtom, (prev) => addDiscountToList(prev, productId, discount));
  }
);

export const removeProductDiscountAtom = atom(
  null,
  (
    _get,
    set,
    { productId, discount }: { productId: string; discount: Discount }
  ) => {
    set(productsAtom, (prev) =>
      removeDiscountFromList(prev, productId, discount)
    );
  }
);
